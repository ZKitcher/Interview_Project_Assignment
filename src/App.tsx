...
function App() {
    ...
    async function addWeatherItem(newItem: string) {
        ...
        const url = `http://api.openweathermap.org/data/2.5/weather?zip=${newItem}&appid=${openweatherAPIKey}`;
    ...
    useEffect(() => {
        ...
        function listCurrentPosition() {
            return new Promise<LocProp>(responseObj => {
                const coordinates: LocProp = {
                    latitude: 0,
                    longitude: 0,
                    error: true,
                };
        ...
        listCurrentPosition().then(res => {
            ...
            const url = `http://api.geonames.org/findNearbyPostalCodesJSON?lat=${
                res.latitude
            }&lng=${
                res.longitude
            }&username=${
